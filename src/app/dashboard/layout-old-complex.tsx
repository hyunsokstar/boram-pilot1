...
export default function DashboardLayout({ children }: { children: ReactNode }) {
    ...
    const currentTabAreas = useMemo<TabAreas>(() => {
        ...
        console.log('currentTabAreas 계산:', {
            hasAnyTabs,
            existingLeft: safeTabAreas.left.map(t => t.label),
            newTabs: newTabs.map(t => t.label),
            finalLeft: [...safeTabAreas.left, ...newTabs].map(t => t.label)
        });
    ...
    useEffect(() => {
        // 탭 개수나 ID가 실제로 변경된 경우에만 실행
        const tabIds = tabs.map(t => t.id).sort().join(',');
        console.log('탭 목록 변경됨, Zustand로 자동 활성 탭 설정:', tabIds);
    ...
    }, [tabs.map(t => t.id).join(',')]); // 탭 ID 목록이 변경될 때만 실행
    ...
    }, [activeTabId, currentTabAreas]); // setStoreActiveTabByArea 제거
    ...
    }, [splitMode, currentTabAreas]); // 함수 dependency들 제거    // 드래그 시작 핸들러
    ...
    const handleTabMove = (tabId: string, fromArea: TabArea, toArea: TabArea, targetIndex?: number) => {
        ...
        // 중요: 탭 스토어의 order 값도 업데이트
        const allCurrentTabs = [
            ...(currentTabAreas.left || []),
            ...(currentTabAreas.center || []),
            ...(currentTabAreas.right || [])
        ].filter(tab => tab && tab.id);

        // 목표 영역의 탭들에서 가장 큰 order 값 찾기
        const targetAreaTabs = currentTabAreas[toArea] || [];
        const maxOrderInTargetArea = targetAreaTabs.length > 0
            ? Math.max(...targetAreaTabs.map(t => t.order || 0))
            : 0;

        // 이동된 탭의 order를 목표 영역에서 가장 크게 설정
        const { updateTab } = useTabStore.getState();
        updateTab(tabId, { order: maxOrderInTargetArea + 1 });

        console.log('handleTabMove 완료: order 업데이트됨', {
            toArea,
            tabId,
            newOrder: maxOrderInTargetArea + 1,
            wasActiveInFromArea
        });
    };
    ...
    const handleTabAreaDrop = (tabId: string, targetArea: TabArea) => {
        ...
        console.log('handleTabAreaDrop 완료: order 업데이트됨', {
            targetArea,
            tabId,
            newOrder: maxOrderInTargetArea + 1
        });
    ...
    const handleDropZoneDrop = (tabId: string, position: DropPosition) => {
        ...
        console.log('handleDropZoneDrop 완료: order 업데이트됨', {
            targetArea,
            tabId,
            newOrder: maxOrderInTargetArea + 1
        });
